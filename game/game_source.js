function TestRagdoll() {
   var water_height_percent = parseInt(document.getElementById('water_height').value) / 100;
   var oldvalue;
   var b2Vec2 = Box2D.Common.Math.b2Vec2, b2AABB = Box2D.Collision.b2AABB, b2BodyDef = Box2D.Dynamics.b2BodyDef,
       ...
       b2World = Box2D.Dynamics.b2World, b2MassData = Box2D.Collision.Shapes.b2MassData,
       b2PolygonShape = Box2D.Collision.Shapes.b2PolygonShape, b2CircleShape = Box2D.Collision.Shapes.b2CircleShape,
       b2DebugDraw = Box2D.Dynamics.b2DebugDraw, b2MouseJointDef = Box2D.Dynamics.Joints.b2MouseJointDef,
   ...
   fixDef.shape.SetAsBox(canvas_width / 30 / 2, 2);
   bodyDef.position.Set(canvas_width / 30 / 2, canvas_height / 30 + 1.8);
   world.CreateBody(bodyDef).CreateFixture(fixDef);
   bodyDef.position.Set(canvas_width / 30 / 2, -1.8);
   world.CreateBody(bodyDef).CreateFixture(fixDef);
   fixDef.shape.SetAsBox(2, canvas_height / 30 / 2);
   bodyDef.position.Set(-1.8, canvas_height / 30 / 2);
   world.CreateBody(bodyDef).CreateFixture(fixDef);
   bodyDef.position.Set(canvas_width / 30 + 1.8, canvas_height / 30 / 2);
   world.CreateBody(bodyDef).CreateFixture(fixDef);
   var m_physScale = 30;
   var fuliController;
   ...
   function createController() {
      ...
      while (bodyList) {
         var body = bodyList.GetUserData();
         fuliController.AddBody(bodyList);
         bodyList = bodyList.GetNext();
      }
   }

   for (var i = 0; i < 10; i++) {
      var startX = canvas_width / 10 * i + Math.random() * 20;
      var startY = 20 + Math.random() * 50;
      bodyDef.type = b2Body.b2_dynamicBody;
      circ = new b2CircleShape(12.5 / m_physScale);
      fixDef.shape = circ;
      fixDef.density = 1.0 + i;
      fixDef.friction = 0.4;
      ...
      var torso1 = world.CreateBody(bodyDef);
      torso1.CreateFixture(fixDef);
      box = new b2PolygonShape();
      box.SetAsBox(15 / m_physScale, 10 / m_physScale);
      fixDef.shape = box;
      bodyDef.position.Set(startX / m_physScale, (startY + 43) / m_physScale);
      var torso2 = world.CreateBody(bodyDef);
      torso2.CreateFixture(fixDef);
      box.SetAsBox(15 / m_physScale, 10 / m_physScale);
      fixDef.shape = box;
      bodyDef.position.Set(startX / m_physScale, (startY + 58) / m_physScale);
      var torso3 = world.CreateBody(bodyDef);
      torso3.CreateFixture(fixDef);
      fixDef.density = 1.0 + i;
      fixDef.friction = 0.4;
      fixDef.restitution = 0.1;
      box = new b2PolygonShape();
      ...
      var upperArmL = world.CreateBody(bodyDef);
      upperArmL.CreateFixture(fixDef);
      box = new b2PolygonShape();
      box.SetAsBox(18 / m_physScale, 6.5 / m_physScale);
      fixDef.shape = box;
      bodyDef.position.Set((startX + 30) / m_physScale, (startY + 20) / m_physScale);
      var upperArmR = world.CreateBody(bodyDef);
      upperArmR.CreateFixture(fixDef);
      fixDef.density = 1.0 + i;
      fixDef.friction = 0.4;
      fixDef.restitution = 0.1;
      box = new b2PolygonShape();
      ...
      var lowerArmL = world.CreateBody(bodyDef);
      lowerArmL.CreateFixture(fixDef);
      box = new b2PolygonShape();
      box.SetAsBox(17 / m_physScale, 6 / m_physScale);
      fixDef.shape = box;
      bodyDef.position.Set((startX + 57) / m_physScale, (startY + 20) / m_physScale);
      var lowerArmR = world.CreateBody(bodyDef);
      lowerArmR.CreateFixture(fixDef);
      fixDef.density = 1.0 + i;
      fixDef.friction = 0.4;
      fixDef.restitution = 0.1;
      box = new b2PolygonShape();
      ...
      var upperLegL = world.CreateBody(bodyDef);
      upperLegL.CreateFixture(fixDef);
      box = new b2PolygonShape();
      box.SetAsBox(7.5 / m_physScale, 22 / m_physScale);
      fixDef.shape = box;
      bodyDef.position.Set((startX + 8) / m_physScale, (startY + 85) / m_physScale);
      var upperLegR = world.CreateBody(bodyDef);
      upperLegR.CreateFixture(fixDef);
      fixDef.density = 1.0 + i;
      fixDef.friction = 0.4;
      fixDef.restitution = 0.1;
      box = new b2PolygonShape();
      ...
      var lowerLegL = world.CreateBody(bodyDef);
      lowerLegL.CreateFixture(fixDef);
      box = new b2PolygonShape();
      box.SetAsBox(6 / m_physScale, 20 / m_physScale);
      fixDef.shape = box;
      bodyDef.position.Set((startX + 8) / m_physScale, (startY + 120) / m_physScale);
      var lowerLegR = world.CreateBody(bodyDef);
      lowerLegR.CreateFixture(fixDef);
   ...
   for (var j = 1; j <= 10; j++) {
      ...
      fixDef.shape = box;
      bodyDef.position.Set((10 * j) / m_physScale, (canvas_height - 200 + 20 * j) / m_physScale);
   ...
   for (var k = 1; k <= 10; k++) {
      ...
      fixDef.shape = box;
      bodyDef.position.Set((canvas_width - 10 * k) / m_physScale, (canvas_height - 200 + 20 * k) / m_physScale);
      head = world.CreateBody(bodyDef);
      head.CreateFixture(fixDef);
   }
   bodyDef.type = b2Body.b2_staticBody;
   ...
   fixDef.shape.SetAsBox(
       150 / m_physScale
       , 3 / m_physScale);
   bodyDef.position.x = canvas_width / m_physScale / 2;
   bodyDef.position.y = canvas_height / m_physScale / 2;
   world.CreateBody(bodyDef).CreateFixture(fixDef);
   bodyDef.position.x = canvas_width / m_physScale / 4;
   bodyDef.position.y = canvas_height / m_physScale / 4;
   world.CreateBody(bodyDef).CreateFixture(fixDef);
   bodyDef.type = b2Body.b2_dynamicBody;
   fixDef.density = 10.0;
   fixDef.friction = 0.5;
   ...
   function handleMouseUp()    //鼠标松开或离屏事件
   {
      isMouseDown = false; //更新状态位
      mouseX = undefined;
      mouseY = undefined;
      document.removeEventListener("mousemove", handleMouseMove, true); //移除
      document.removeEventListener("touchmove", handleMouseMove, true);
   }

   document.addEventListener("mouseup", handleMouseUp, true);  //监听
   document.addEventListener("touchend", handleMouseUp, true);  //监听

   function handleMouseMove(e)  //鼠标移动事件或滑屏事件，更新坐标
   {
      var clientX, clientY;
      if (e.clientX) {
         clientX = e.clientX; //e.clientX获取鼠标在窗口中的X坐标
         clientY = e.clientY; //e.clientY获取鼠标在窗口中的Y坐标
      } else if (e.changedTouches && e.changedTouches.length > 0) {
         var touch = e.changedTouches[e.changedTouches.length - 1];
         clientX = touch.clientX;  //获取鼠标移动后的坐标X
         clientY = touch.clientY;  //获取鼠标移动后的坐标Y
      } else {
         return;
      }
      mouseX = (clientX - canvasPosition.x) / 30;
      mouseY = (clientY - canvasPosition.y) / 30;
      e.preventDefault();  //阻止操作的默认行为
   }

   function getBodyAtMouse() {
      mousePVec = new b2Vec2(mouseX, mouseY);
      var aabb = new b2AABB();  //AABB范围框
   ...
   //定时刷新
   function update() {
      if (isMouseDown && (!mouseJoint))  //点击
      {
         var body = getBodyAtMouse();//获取选中
         if (body) {
            var md = new b2MouseJointDef();
            md.bodyA = world.GetGroundBody();  //绑定
            md.bodyB = body;  //关节连接的刚体
            md.target.Set(mouseX, mouseY);  //关节
      ...
      water_height_percent = parseInt(document.getElementById('water_height').value) / 100
